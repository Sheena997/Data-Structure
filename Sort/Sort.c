...
void CountSort(int* a, int n)
{
	...
	memset(tmp, 0, sizeof(int)*range);//使tmp计数必须变为0因为初始值是0。
	for (int i = 0; i < n; ++i)
	{
		tmp[a[i] - min]++;
	}
...
{
	int s[] = { 9,8,7,6,5,4,3,2,1 };
	MergeSort2(&s, sizeof(s) / sizeof(int));
	Print(s, sizeof(s) / sizeof(int));
}
void CountSortTest()
{
	int s[] = { 101,102,101,109,109,101,103,102,104 };
	CountSort(&s, sizeof(s) / sizeof(int));
	Print(s, sizeof(s) / sizeof(int));
}